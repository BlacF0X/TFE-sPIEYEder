...
import threading
import detecam,center_cam,recepcam
import time
import db
import random
...
def start_detection():
    threading.Thread(
        target=recepcam.receive_cam,
        args=(5002, frame_queue_l),
        daemon=True
    ).start()

    threading.Thread(
        target=recepcam.receive_cam,
        args=(5003, frame_queue_r),
        daemon=True
    ).start()
...
def create_app():
    ...
    def api_sensors():
        ...
        data = {
            ...
            "object_count": angles,
            "depth": psutil.sensors_temperatures()['cpu_thermal'][0].current,
